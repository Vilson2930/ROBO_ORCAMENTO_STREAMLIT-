...
def preparar_evolucao_faturas(df):
    ...
    resultado = base.groupby(coluna_fatura)[valor_col].sum().reset_index()
...
def criar_resumo_cliente(gasto_total, df_grafico, diagnostico, compromissos):
    if df_grafico.empty:
...
def montar_tabela_faturas_processadas(documentos, df_faturas):
    valores = {}

    if df_faturas is not None and not df_faturas.empty:
        for _, row in df_faturas.iterrows():
            valores[str(row["Fatura"])] = row["Valor"]

    linhas = []

    for doc in documentos:
        arquivo = str(doc.get("arquivo", "-"))
        valor_detectado = 0

        for nome, valor in valores.items():
            if arquivo == nome or nome in arquivo or arquivo in nome:
...
def score_bar(score):
    ...
    st.markdown(
        ...
        <div class="score-box">
            ...
            <div class="score-value">{score_num:.0f}/100</div>
            <div class="score-bar-bg">
                <div class="score-bar-fill" style="width:{score_num}%;"></div>
            </div>
            <div class="score-label">{classificar_score(score_num)}</div>
        </div>
        """,
        unsafe_allow_html=True
    )
...
if analisar:
    if not pdfs:
        ...
        with st.spinner("Analisando suas faturas..."):
            ...
            st.session_state["resultado_compromissos"] = resultado_compromissos
...
if pagina == "⚙️ Auditoria":
    ...
    df_parcelamentos = st.session_state.get("df_parcelamentos", pd.DataFrame())
    ...
    st.subheader("Parcelamentos encontrados")
    st.dataframe(df_parcelamentos, use_container_width=True)
...
if pagina == "🏠 Resumo":
    ...
    with c3:
        card_html(
            "Parcelas futuras",
            moeda(resultado_compromissos["valor_restante_total"]),
    ...
    resumo_cliente = criar_resumo_cliente(
        gasto_total,
        df_grafico,
        diagnostico,
        resultado_compromissos
    ...
    with a3:
        insight_card(
            ...
            gerar_resumo_executivo_compromissos(resultado_compromissos),
            "#EF4444" if resultado_compromissos["classificacao"] in ["ALTO", "CRÍTICO"] else "#22C55E"
        )
...
elif pagina == "💳 Compromissos":

    st.header("Compromissos financeiros")
    ...
    with c2:
        card_html(
            "Impacto mensal",
            moeda(resultado_compromissos["impacto_mensal"]),
    ...
    with c3:
        card_html(
            "Peso mensal",
            f"{resultado_compromissos['impacto_mensal_percentual']:.1f}%",
    ...
    with c4:
        card_html(
            ...
            moeda(resultado_compromissos["maior_parcela_valor"]),
            resultado_compromissos["maior_parcela_compra"],
            "#3B82F6"
        )
    ...
    insight_card(
        resultado_compromissos["nivel_risco"],
        gerar_texto_capacidade(resultado_compromissos),
        "#EF4444" if resultado_compromissos["classificacao"] in ["ALTO", "CRÍTICO"] else "#22C55E"
    )

    st.markdown('<div class="section-title">5 maiores saldos futuros</div>', unsafe_allow_html=True)
    ...
    st.markdown('<div class="section-title">5 maiores impactos mensais</div>', unsafe_allow_html=True)

    top_impacto = gerar_top_impacto_mensal(df_parcelamentos, top=5)
    ...
    tabela_docs = montar_tabela_faturas_processadas(documentos, df_faturas)
    ...
    insight_card(
        ...
        criar_resumo_cliente(gasto_total, df_grafico, diagnostico, resultado_compromissos),
        "#22C55E"
    )
    ...
    if resultado_compromissos["classificacao"] in ["ALTO", "CRÍTICO"]:
        insight_card(
